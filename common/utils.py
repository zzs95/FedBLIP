...
def decode_from_logits(tokenizer, logits):
    """
    logits: [B, L, V] = lm_output.logits
    return: List[str] 长度为 B
    """
    sep_id = tokenizer.sep_token_id   # 102
    pad_id = tokenizer.pad_token_id   # 0
    # 1. greedy 解码到 token id
    pred_ids = logits.argmax(dim=-1)   # [B, L]

    pred_texts = []
    for i in range(pred_ids.size(0)):
        ids = pred_ids[i].tolist()     # 当前样本的 [L]

        # 2. 先按 PAD 截断（找到第一个 PAD）
        ...
        # 3. 再按 SEP 截断（找到第一个 SEP）
        if sep_id is not None and sep_id in ids:
            first_sep = ids.index(sep_id)
            ids = ids[:first_sep]      # 不包含 SEP 本身
