...
def anno_files(setname):
    if setname == 'Brown':
        data_split = pd.read_excel('Browh_splits.xlsx', index_col=False) 
        abntext_path = 'brown_CTPA_PE_disease_finding_56.xlsx'
        image_root = 'CTPA_process_Brown'
    elif setname == 'INSPECT':
        data_split = pd.read_excel('INSPECT_splits.xlsx', index_col=False) 
        abntext_path = 'inspecta_CTPA_PE_disease_finding_56.xlsx'
        image_root = 'CTPA_process_INSPECT'
    elif setname == 'JHU':
        data_split = pd.read_excel('JHU_splits.xlsx', index_col=False)   
        abntext_path = 'jhu_CTPA_PE_disease_finding_56.xlsx'
        image_root = 'CTPA_process_JHU'
    else:
